...
```{r setup}
airlineData <- read.csv("C:/Users/jghuynh/Documents/R/Linear_Regression/Project_Aviation/airlineData.csv", stringsAsFactors = T, na.strings = c(""))
library(zoo)
library(dplyr)
library(tidyverse)
library(stringr)
...
```
It appears that all the ratings variables are significiant.
This model answered our questions on what factors contribute most to the overall quality of the airlines. We trust the model because intuitively it makes sense. If the people at the terminals and before boarding are kind, then of course the overall quality increases.
...
Coef.Lasso <- coef(final.model)[1:7,]
